 ...
 * 目前缓存过期机制采用被动刷新，即调用方调用get()，才判断缓存是否过期，依据为LocalCacheWrapper包装类中的expireTime和initTime
 * @author guoyunfeng
 */
public class LocalCache <K, V> implements LocalCacheInterface<K, V> {
     ...
     * @param flush
     * @return
     * @throws NoSuchKeyException 如缓存没有键，则抛出NoSuchKeyException
     * @throws CacheExpiredException 如缓存过期，则抛出CacheExpiredException
     */
    @Override
